...
importAll(require.context('./css/', true, /\.css$/)); //importAll(require.context('./img/', true, /\.(png|svg|jpg|jpeg|gif|webp)$/));
//importAll(require.context('./html/', true, /\.html$/));

importAll(require.context('./assets/', true, /\.pdf$/));
importAll(require.context('./media/', true, /\.mp4$/)); // At build-time cache will be populated with all required modules.