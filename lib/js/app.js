...
import { logger, calculate_age, wrapper_exec, wrapper_no_exec, debounce, throttle, scroll_progress, generate_dark_color_hex, form_submit_success, form_submit_error, ajax, media_queries } from "./Business_Logic/Functions.js";
import Skill_Rating from "./Business_Logic/SkillRating.js";
import Project from "./Business_Logic/Project.js";
import TagCloud from "./Business_Logic/TagCloud.js"; //import Formspree from "./Business_Logic/Formspree.js";
...
const App = {
  init() {
    ...
    document.addEventListener("DOMContentLoaded", function () {
      console.log("DOMContentLoaded Successfully"); //UI.toggler_menu_icon_switch();

      (function () {
        // Lazy load images
        const init_lazy_imgs = [].slice.call(UI.lazy_imgs); // Lazy load image sources (picture tag)

        const init_lazy_sources = [].slice.call(UI.lazy_sources); // Lazy load background images in CSS
        ...
        if ("IntersectionObserver" in window) {
            ...
            entries.forEach(function (entry) {
              if (entry.isIntersecting) {
                new Promise(function (resolve, reject) {
                  ...
                  recaptchaCallback(function () {
                    const grecaptcha_check = function () {
                      if (UI.grecaptchas.length > 0) {
                        ...
                        grecaptcha_observer.unobserve(entry.target); //console.log(grecaptcha);
                  ...
                  return console.log("Error in script delay promise: ", err);
                });
              }
          ...
          init_grecaptchas.forEach(function (element) {
      ...
      UI.my_form_button.setAttribute("disabled", "disabled"); // Set new poster image if on mobile device
      ...
      window.addEventListener("resize", debounce(function () {
        UI.add_poster_img_sm();
        UI.load_bg_vid();
      }, 500)); // Delay load of non-essential scripts

      setTimeout(function () {
        media_queries(window.matchMedia("(min-width: 768px)"), function () {
      ...
      }, 10000); //const script_delay = 
      // Loads Google Preview on click only

      let google_preview_btn_is_clicked = false;

      (function () {
        UI.google_preview_modal_btn.addEventListener("click", function () {
      ...
      let scroll_timer; // Simulates another click event after first if scrolled to top 
      // Due to dynamic height changes in body anchors can sometimes not scroll to accurate positions

      UI.anchor_links.forEach(function (link) {
      ...
      const header_timer = function () {
        return scroll_timer = window.setTimeout(function () {
          //is_scrolling = false;
          if (!show_header) {
      ...
      (function () {
        const side_menu_toggler = function () {
        ...
        const mobile_menu_toggler = function () {
          if (!UI.toggler_btn.classList.contains("collapsed")) {
        ...
        const mq_menu_toggler = function () {
          const mq_limits = [window.matchMedia("(max-width: 767.98px)"), window.matchMedia("(min-width: 768px) and (max-width: 991.98px)")];
      ...
      UI.header_vid.addEventListener("ended", function () {
        UI.replace_vid_bg();
      ...
      UI.header_vid.addEventListener("animationend", function () {
        setTimeout(function () {
          UI.replace_vid_bg();
          UI.header_vid.remove();
          header_vid_ended = true;
      ...
      (function () {
        ...
        const sticky_header_throttle_wrapper = throttle(function () {
          header_transform();
        }, 100); // Adjusts header to match screen size if resized

        window.addEventListener("resize", debounce(function () {
          header_transform();
          show_header = true;
          clearTimeout(scroll_timer);
        }, 200)); // Stops the header timer when mouse hovers over the header

        UI.header.addEventListener("mouseover", function () {
          show_header = true;
          clearTimeout(scroll_timer);
        }); // Stops the header timer when mouse moves over the header

        UI.header.addEventListener("mousemove", function () {
          show_header = true;
          clearTimeout(scroll_timer);
        }); // Stops the header timer when screen is touched on the header

        UI.header.addEventListener("touchstart", function () {
          show_header = true;
          clearTimeout(scroll_timer);
        }, {
          passive: true
        ...
        UI.header.addEventListener("touchmove", function () {
          show_header = true;
          clearTimeout(scroll_timer);
        }, {
          passive: true
        ...
        UI.header.addEventListener("mouseout", function () {
        ...
        UI.header_btns.forEach(function (btn) {
        ...
        UI.header_links.forEach(function (link) {
        ...
        document.addEventListener("scroll", throttle(function () {
          if (document.documentElement.scrollTop > scroll_limit || window.pageYOffset > scroll_limit) show_header = false;else show_header = true; // Clear previous timer and reset

          clearTimeout(scroll_timer); // Hides the header on scroll stop or shows while scrolling or hovering on element (debounces while scrolling)
      ...
      UI.my_age.innerHTML = calculate_age(); // Adds a fade in and out effect when clicking the button in my bio

      UI.summary_btn.addEventListener("click", function () {
        UI.change_about_info();
      });
      /*** MY SERVICES SECTION ***/

      let my_carousel_btn_click = false; // Changes carousel horizontal scroll amount depending on the screen size

      (function () {
        let scroll_amt = 360;
        const mq_limits = [window.matchMedia("(max-width: 320.98px)"), window.matchMedia("(min-width: 321px) and (max-width: 575.98px)"), window.matchMedia("(min-width: 576px) and (max-width: 767.98px)"), window.matchMedia("(min-width: 768px) and (max-width: 991.98px)"), window.matchMedia("(min-width: 992px) and (max-width: 1199.98px)"), window.matchMedia("(min-width: 1200px) and (max-width: 1399.98px)"), window.matchMedia("(min-width: 1400px)")];

        const scroll_amt_modifier = function () {
          media_queries(mq_limits[0], function () {
            return scroll_amt = 274;
          }, null);
          media_queries(mq_limits[1], function () {
            return scroll_amt = 296;
          }, null);
          media_queries(mq_limits[2], function () {
            return scroll_amt = 360;
          }, null);
          media_queries(mq_limits[3], function () {
            return scroll_amt = 656 / 2;
          }, null);
          media_queries(mq_limits[4], function () {
            return scroll_amt = 720 / 2;
          }, null);
          media_queries(mq_limits[5], function () {
            return scroll_amt = 980 / 3;
          }, null);
          media_queries(mq_limits[6], function () {
      ...
      (function () {
        let tagcloud_radius;
        const mq_limits = [window.matchMedia("(max-width: 320.98px)"), window.matchMedia("(min-width: 321px) and (max-width: 575.98px)"), window.matchMedia("(min-width: 576px) and (max-width: 767.98px)"), window.matchMedia("(min-width: 768px) and (max-width: 991.98px)"), window.matchMedia("(min-width: 992px)")];
        ...
        const tagcloud_loader = function (TagClouds) {
          ...
          let myTags = ['OOP', 'SOC / MVC', 'REST-APIs', 'Data-Structures', 'Continuous-Integration', 'UI / UX', 'Testing', 'Version-Control', 'Debugging', 'Algorithms', 'App-Development', 'Responsive-Design', 'Security', 'Optimization', 'Customer-Service']; // Render a default tag cloud
          ...
          UI.tagcloud_items = document.querySelectorAll(".tagcloud--item"); // Randomizes tag word colours and adds effects on click

          UI.tagcloud_items.forEach(function (item) {
        ...
        const tagcloud_observer = new IntersectionObserver(function (entries, observer) {
          entries.forEach(function (entry) {
            if (entry.isIntersecting) {
              import("./Business_Logic/TagCloud.js").then(function (module) {
                return module.default;
              }).then(function () {
                tagcloud_loader(TagCloud); // Resets and resizes tag cloud for different screen sizes
                ...
                return console.error("Failed to import TagCloud module: ", err);
              });
            }
      ...
      (function () {
        ...
        const populate_carousel_indicators = function (proj_type) {
          current_project.carousel_img_list.srcs.forEach(function (indicator, index) {
            const btn = document.createElement("button");
            btn.setAttribute("type", "button");
            btn.setAttribute("data-bs-target", "#dev-project-carousel");
            if (index === 0) btn.classList.add("active", "btn"), btn.setAttribute("aria-current", "true");else btn.classList.add("btn");
            btn.setAttribute("aria-label", `Slide ${index + 1}`); // Append to dev or client project depending on parameter set

            if (proj_type === "dev".toLowerCase()) UI.dev_project_carousel_indicator_section.appendChild(btn);else if (proj_type === "client".toLowerCase()) UI.dev_project_carousel_indicator_section.appendChild(btn);
          });
        ...
        const populate_carousel_img_data = function (proj_type) {
          current_project.carousel_img_list.srcs.forEach(function (src, index) {
            const div = document.createElement("div");
            if (index === 0) div.classList.add("carousel-item", "active");else div.classList.add("carousel-item");
            ...
            div.appendChild(img); // Append to dev or client project depending on parameter set

            if (proj_type === "dev".toLowerCase()) UI.dev_project_carousel_inner_section.appendChild(div);else if (proj_type === "client".toLowerCase()) UI.client_project_carousel_inner_section.appendChild(div);
          });
        ...
        const populate_project_notes = function (proj_type) {
          current_project.notes.forEach(function (note, index) {
            const list = document.createElement("li");
            list.innerHTML = note;
            if (index === current_project.notes.length - 1) list.classList.add("fw-bold"); // Append to dev or client project depending on parameter set

            if (proj_type === "dev".toLowerCase()) UI.dev_project_carousel_note_section.appendChild(list);else if (proj_type === "client".toLowerCase()) UI.client_project_carousel_note_section.appendChild(list);
          });
        ...
        const populate_project_tool_icons = function (proj_type) {
          current_project.tool_icon_list.srcs.forEach(function (src, index) {
            const img = document.createElement("img");
            img.setAttribute("loading", "lazy");
            ...
            img.setAttribute("height", "64"); // Append to dev or client project depending on parameter set

            if (proj_type === "dev".toLowerCase()) UI.dev_project_carousel_icon_section.appendChild(img);else if (proj_type === "client".toLowerCase()) UI.client_project_carousel_icon_section.appendChild(img);
          });
        };
        ...
        const reinitialize_el = function () {
          ...
          populate_project_tool_icons("dev"); //console.log("new indicators", UI.dev_project_carousel_indicator_section, "new inner", UI.dev_project_carousel_inner_section);
        ...
        const Alien_Mathvasion = new Project("Alien Mathvasion Game", 2, "https://dnoelmathinvasiongame.netlify.app/html/gamescreen.html", "Play", "https://github.com/DNoel26/Alien_Mathvasion", true);
        ...
        Alien_Mathvasion.add_tool_icons(alien_m_tool_img_ids, alien_m_tool_img_srcs, alien_m_tool_img_alts); // Wix Clone Project

        const Wix_Clone = new Project("Wix Site Clone", 1, "https://dnoelmotorcyclewixclone.netlify.app/", "View", "https://github.com/DNoel26/Wix_Motorcycle_Trial", true);
        ...
        Wix_Clone.add_tool_icons(wix_c_tool_img_ids, wix_c_tool_img_srcs, wix_c_tool_img_alts); // Cyberdise Online Store Project

        const Cyberdise = new Project("Cyberdise Online Store", 2, "https://dnoelcyberdise.herokuapp.com/", "Interact", "https://github.com/DNoel26/Cyberdise-Dynamic-", true);
        ...
        const cyberdise_carousel_img_srcs = ["./img/projects/min/cyberdise-online-store-1-min.webp", "./img/projects/min/cyberdise-online-store-2-min.webp", "./img/projects/min/cyberdise-online-store-3-min.webp", "./img/projects/min/cyberdise-online-store-4-min.webp", "./img/projects/min/cyberdise-online-store-5-min.webp"];
        const cyberdise_carousel_img_alts = [];
        const cyberdise_tool_img_ids = [];
        const cyberdise_tool_img_srcs = ["./img/logos/html5-badge.webp", "./img/logos/css3-badge.webp", "./img/logos/javascript-badge.webp", "./img/logos/handlebars-badge.webp", "/img/logos/mysql-badge.webp", "./img/logos/nodejs-badge.webp", "./img/logos/express-logo.webp", "./img/logos/postman-badge.webp"];
        ...
        Cyberdise.add_tool_icons(cyberdise_tool_img_ids, cyberdise_tool_img_srcs, cyberdise_tool_img_alts); // Movie Database Project

        const Movie_Database = new Project("Movie Database", 2, "https://dnoelmovieapidatabase.netlify.app/", "View", "https://github.com/DNoel26/Movie_Database", true);
        ...
        Movie_Database.add_tool_icons(movie_db_tools_img_ids, movie_db_tools_img_srcs, movie_db_tools_img_alts); // Amazon Clone Project

        const Amazon_Clone = new Project("Amazon Clone", 2, "https://clone-905a7.web.app/", "Interact", "https://github.com/DNoel26/Amazon_React_Clone", true);
        ...
        const amazon_c_carousel_img_srcs = ["./img/projects/min/amazon-clone-1-min.webp", "./img/projects/min/amazon-clone-2-min.webp", "./img/projects/min/amazon-clone-3-min.webp", "./img/projects/min/amazon-clone-4-min.webp"];
        const amazon_c_carousel_img_alts = [];
        const amazon_c_tool_img_ids = [];
        const amazon_c_tool_img_srcs = ["./img/logos/html5-badge.webp", "./img/logos/css3-badge.webp", "./img/logos/javascript-badge.webp", "./img/logos/react-badge.webp", "./img/logos/firebase-badge.webp", "./img/logos/nodejs-badge.webp", "./img/logos/express-logo.webp"];
        ...
        Amazon_Clone.add_tool_icons(amazon_c_tool_img_ids, amazon_c_tool_img_srcs, amazon_c_tool_img_alts); // Real Estate Website Project

        const Real_Estate_Site = new Project("Real Estate Site", 3, "https://presidentialrealtors-dev-static.netlify.app/", "View", "https://github.com/DNoel26/Presidential-Realtors-Static", true);
        ...
        Real_Estate_Site.add_tool_icons(real_estate_tool_img_ids, real_estate_tool_img_srcs, real_estate_tool_img_alts); // Retrieve last project stored in session and execute change project if current project is not empty
        ...
        ;
        UI.dev_project_gallery_btns.forEach(function (btn) {
          btn.addEventListener("click", function () {
            ...
            change_project(); // Store current project in session storage

            sessionStorage.setItem("current_project", JSON.stringify(current_project)); //console.log("old indicators", UI.dev_project_carousel_indicator_section, "old inner", UI.dev_project_carousel_inner_section);

            UI.dev_project_overview.innerHTML = new_inner_html;
            reinitialize_el();
            UI.dev_project_overview.scrollIntoView();
            UI.return_to_dev_gallery_btns.forEach(function (btn) {
              btn.addEventListener("click", function () {
        ...
        (function () {
          ...
          const target_node = UI.dev_project_overview; // Options for the observer (which mutations to observe)
          ...
          const callback = function (mutationsList, observer) {
            ...
            for (const mutation of mutationsList) {
              ...
              } else if (mutation.type === 'subtree') {//console.log('The subtree attribute was modified.');
              }
          ...
          observer.observe(target_node, config); // Later, you can stop observing
          // observer.disconnect();
        })();
      ...
      (function () {
        ...
        const form_api_observer = new IntersectionObserver(function (entries, observer) {
          entries.forEach(function (entry) {
            if (entry.isIntersecting) {
              import("./Business_Logic/API.js").then(function (module) {
                return module.default;
              ...
              .then(function (API) {
                ...
                select_change = function () {
                  const selected_options = document.querySelectorAll("option"); // Adds country flag and phone calling code on country select
                ...
                Country_API.fetch_api().then(function (data) {
                  ...
                  UI.phone.addEventListener("keyup", debounce(function () {
                    user_typed = true;
                  }, 500));
                  UI.country_select.addEventListener("change", debounce(function () {
                    select_change();
                  }, 300));
                }).catch(function (err) {
                  ...
                  return formspree();
                }); // Executes formspree function regardless of promise fulfillment or rejection    
              }).catch(function (err) {
                return console.log("Failed to import API module: ", err);
              });
            }
        ...
        const formspree = function () {
          ...
          const validation_msgs = [function () {
          ...
          Array.prototype.slice.call(UI.forms_need_validation).forEach(function (form, index) {
            (function () {
              // Cycle through each form input/select/text area tags and store or populate with sessionStorage
              form.querySelectorAll(".form-data").forEach(function (data) {
                ...
                if (data.tagName === "SELECT") data.value = sessionStorage.getItem(data.name) || ""; //console.log(data, data.tagName, data.name, data.value);

                select_change(); // Save contact form info in cookies

                data.addEventListener("input", debounce(function () {
            ...
            form.addEventListener("submit", function (event) {
              event.preventDefault(); // Stop multiple submits from occurring 

              event.stopImmediatePropagation();
              import('./Business_Logic/Formspree.js').then(function (module) {
                return module.default;
              }) // uses the default export
              .then(function (Formspree) {
                if (!form.checkValidity()) {
                  ...
                  ajax(My_Form.method, My_Form.url, My_Form.data, success, error, function (status) {
                ...
                return console.error("Failed to import Formspree module: ", err);
              });
            }, false);
          });
        };
